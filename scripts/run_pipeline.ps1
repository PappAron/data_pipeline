# 1. Build Azure Infrastructure
Write-Host "--- Stage 1: Deploying Azure Infrastructure ---" -ForegroundColor Green
terraform "-chdir=../terraform/infra" init
terraform "-chdir=../terraform/infra" apply -auto-approve

$DB_URL = terraform "-chdir=../terraform/infra" output -raw databricks_url
$KV_ID  = terraform "-chdir=../terraform/infra" output -raw key_vault_id
$KV_URI = terraform "-chdir=../terraform/infra" output -raw key_vault_uri

Write-Host "Waiting 60 seconds for Databricks DNS propagation..." -ForegroundColor Cyan
Start-Sleep -Seconds 6

Write-Host "--- Stage 2: Deploying Databricks Resources ---" -ForegroundColor Green
terraform "-chdir=../terraform/databricks" init
terraform "-chdir=../terraform/databricks" apply `
  -var="databricks_url=$DB_URL" `
...
$connString = (terraform "-chdir=../terraform/infra" output -raw storage_connection_string).Trim()
$omdbKey = az keyvault secret show --name $SECRET_OMDB --vault-name $VAULT_NAME --query value -o tsv
$env:CONNECTION_STRING = $connString
...
$WS_URL = (terraform "-chdir=../terraform/infra" output -json databricks_url | ConvertFrom-Json)
$JOB_ID = (terraform "-chdir=../terraform/databricks" output -json databricks_job_id | ConvertFrom-Json)
